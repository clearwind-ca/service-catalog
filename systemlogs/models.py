...
class SystemLog(models.Model):
    ...
    level = models.IntegerField(choices=[(k, v) for k, v in messages.DEFAULT_LEVELS.items()])
    created = models.DateTimeField(auto_now_add=True)
    message = models.TextField()

    def get_target(self):
        return apps.get_model(self.target_app_label, self.target_model_name).objects.get(slug=self.target_slug)
